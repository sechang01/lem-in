...
/*   Updated: 2019/01/19 22:38:08 by sechang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		test_output2(t_lemin *input, t_rooms *head, char *name)
{
	...
	i = 0;
	printf("TEST OUTPUT COMMENCE2: %s \n", name);
	r_tmp = input->anthead;
	if (input->anthead)
		printf("Head: %s\n", input->anthead->name);
	if (input->anttail)
...
//		test_output3(input);
	...
	input->state = 1;
	test_output2(input, input->anthead, "Closest");
	//	printf("Headstate: %d\n", headstate);
	//	printf("Tailstate: %d\n", tailstate);
	//	printf("curr node: %s\toccupied?:%d\n", curr->name, curr->occupied);
	disttmp = 2147483647;
	closest = (curr == input->start) ? NULL : curr; // ((t_rooms *)(hash_lookup(l_tmp->link, input->hasht))->data);
//	closest = curr;
	while (l_tmp)
	{
	...
	if (closest != curr && closest != NULL)
	{
		...
		if (headstate == 1)
		{
			linkem(input, h_tmp, curr, closest, 1);
		}
		else if (tailstate == 1)
		{
			linkem(input, h_tmp, curr, closest, 2);
		}
		else
		{                                                                 
	...
	if (input->anttail)
		printf("Tail: %s\n", input->anttail->name);
	printf("\t\t\t\t===========>L%d-%s ", input->antnum, closest->name);
		input->antnum++;
	}
...
void	start_nexus(t_lemin *input)
{
	...
	printf("\nSTART NEXUS\n");
	test_output2(input, input->anthead, "Start Nexus Begin");
	l_tmp = input->start->linx;
	while (l_tmp)
	{
		if (key == 0)
...
//				r_tmp->next = NULL;
			...
			test_output2(input, input->anthead, "Start Nexus Loop");
		}
		else
			break;
		printf("~~~~~~~~\n\n\n");
...
//	exit(1);
	...
	{
		printf("-------------------------------------------------\n\tTURN: %d START! -Ants out: %d Ants in: %d\n\n\n",
		...
		{
			if (input->prevroom)
				printf("old prevroom: %s\n", input->prevroom->name);
	//		input->prevroom = closest_node(input, r_tmp);
			if ((close_tmp = closest_node(input, r_tmp)))
				input->prevroom = close_tmp;
			if (input->prevroom)
				printf("new prevroom: %s\n", input->prevroom->name);
				rr_tmp = r_tmp;			// in the case of ant-can't-move
				r_tmp = r_tmp->next;	// next is set to NULL!
			if (input->state != 33)
				rr_tmp->next = NULL;
		}
		if (input->anttail)
		...
		start_nexus(input);
		printf("\n\n\nTURN %d COMPLETE!\n\n\n", input->turn);
		test_output2(input, input->anthead, "Gogo Ants");
...
//		while(1);
	...
	/*	if (input->turn > input->ants_in + 100)
		{
			printf("Entrances: %d\n", input->entrances);
			printf("Shortest Path: %d\n", input->start->dist);
			test_output2(input, input->anthead, "Exit Case");
			exit(1);
		} */
		printf("Shortest Path: %d\n", input->start->dist);
		input->turn++;
	}
	printf("----------------------------------------------\n\t\t\tTURN: %d    -	Ants out: %d     Ants in: %d\n\n", input->turn, input->ants_out, input->ants_in);
...
int		room_dist(t_lemin *input)
{
	...
	printf("start dist:%d\n", input->start->dist);
	while (input->state != 0 || input->start->dist == -1)
	{
		r_tmp = input->roomhead;
		printf("R_TMP = %s\n", input->roomhead->name);
		printf("tmpdst=%d\n", tmpdst);
...
int			main()
{
	...
	printf("HEREYE\n");
	printf("startname=%c\n", input->start->name);
	if (!input->nbrofrooms || !input->start || !input->end->name)
	{
		printf("NO VALUE\n");
		if (!(ft_strcmp(input->start->name, input->end->name)))
		{
			printf("ERROR CASE EXIT\n");
			exit(1);
		}
	}
	printf("HOLA");
